...
## Q. 预览 UX 里程碑（微博式图文混排 · 本轮新增，双端）

新增一个跨端配置 **`autoPlayVideo`**（`PickerConfig`，默认 `false`，向后兼容——旧调用不受影响）。
其余全是**原生 UX 增强**，不改契约。我已在 iOS 模拟器自查过「呈现 / 翻页页码更新 / 加载淡入」，
但动画手感、配色、时长这类**主观项**需要你在真机定稿（见文末备注）。

**契约**
- [ ] `previewAssets(config: PickerConfig(autoPlayVideo: true))` 视频页进入即自动播放；`false`（默认）时不自动播
- [ ] 不传 `autoPlayVideo` 的旧调用行为完全不变

**iOS / Android 通用体验**
- [ ] **飞入 hero**：点缩略图，预览从该缩略图位置放大飞入（不再是居中淡入）；下拉/关闭飞回原缩略图
- [ ] **翻页后关闭**：翻到别的图后再关闭，是干净淡出（不会飞回错误的缩略图格子）
- [ ] **视频自动播放**（`autoPlayVideo=true`）：只有**当前页**的视频自动播；左右相邻页不会偷偷出声；翻走上一个视频立即停
- [ ] **加载淡入**：翻页时图片是「先低清/占位 → 交叉淡入清晰图」，没有「空白闪一下」
- [ ] **底部页码圆点**：2–8 张图文时底部出现小圆点、随翻页高亮当前页；>8 张时不显示底部圆点（顶部 `n/m` 计数已够）；**视频页**不显示圆点（让开播放进度条）
- [ ] **双击放大**：在图片上双击可放大到点击位置（再次双击复原）

**iOS 侧重**
- [ ] 视频有播放/暂停、当前时间 / 总时长、可拖动进度条（本轮前置修复，一并回归）

**Android 侧重**
- [ ] **首页就是视频**时也能自动播放（此前初始页不触发，已修）
- [ ] **快速下滑**也能关闭预览（fling，不必长距离拖动）
- [ ] **缩放橡皮筋**：捏到最大再捏有轻微回弹；缩放态下**甩动**有惯性滑行且不会把图甩出边界
- [ ] 分享按钮图标是自定义 `ic_share`（非系统灰色 `ic_menu_share`）

> **主观项（我给了合理默认，最终以你真机为准）**：飞入/淡出时长（iOS 0.3s）、交叉淡入时长（≈180–200ms）、
> 圆点尺寸/间距/透明度、自动播放是否**静音**（当前：带声、播一次、不循环——若要信息流式静音循环告诉我，加开关很快）、
> 圆点 vs 顶部计数的取舍（当前：≤8 圆点 / >8 顶部计数）。这些改起来都很快。

## R. 健壮性 & 「帖子查看」页（本轮新增）

首页「发帖」区新增入口 **「帖子查看（信息流·图文视频混排）」**，一屏内 5 条帖子覆盖各组合，逐一点开验证。
资源全用网络（大陆可达优先，写死在 `example/lib/pages/post_feed_page.dart` 顶部常量，打不开就换）。

**帖子查看页（逐帖点开预览）**
- [ ] ① 单图：单张全屏、双击放大到点、下拉飞回
- [ ] ② 多图（6 张）：翻页 + **底部圆点**随页高亮
- [ ] ③ 图文+视频混排：翻到视频页**自动播放**、翻走停
- [ ] ④ 多视频（含 34MB 大文件）：逐页自动播；大文件**缓冲时有转圈**（iOS/Android 都应显示）
- [ ] ⑤ 坏链帖：缩略图显示破图占位；点开原生预览显示 **"加载失败，点按重试"**，点重试会重新拉取

**坏链/加载/缓冲兜底（本轮补的，对应你问的三点）**
- [ ] 坏图/坏视频进原生预览：显示错误占位 + **点按重试**（不再静默空白/冻结）
- [ ] 图片加载中：Android 有转圈（iOS 本就有）
- [ ] 视频缓冲卡顿：画面上有**缓冲转圈**（iOS 观察 timeControlStatus；Android ExoPlayer 内置）；缓冲回填后自动继续
- [ ] 视频坏链点「重试」多次：不崩溃、不卡顿累积（已修重试时的播放器泄漏）

**原图导出（对应你问的「传后端」）**
- [ ] `exportAsset(assetId:, format:, original: true)` 在 **iOS** 返回**真原图/原视频**（不再降质为 1600×1600 / ≤1080p）；Android 本就是原图
- [ ] `original: false`（默认）行为不变；旧调用不受影响
- [ ] 缩略图仍由 `MediaItem.thumbnailPath`（200×200）直接提供，无需额外导出

> **给后端的建议用法**：列表/上传缩略走 `thumbnailPath`；要归档/原画质就 `exportAsset(..., original: true)`。
> `原图` 开关（`isOriginalPhoto`）目前只是**回传给你的建议标志**，不影响 `exportAsset`——是否传 `original: true` 由你按它决定。
