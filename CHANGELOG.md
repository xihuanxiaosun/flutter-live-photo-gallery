...
**预览模式（`showRadio: false`，纯预览/微信式）**
- 隐藏整条顶栏（关闭/计数/分享/下载按钮均不显示）；**单击图片即关闭预览**（放大态与视频页除外）
- 预览页**状态栏为白色**（浅色内容，适配黑底）
- 底部页码圆点**常驻**（不随手势淡出，下拉关闭时才一起淡出）
- 分享按钮已移除（两种模式均不显示）

**长按网络图保存（需 `showDownloadButton: true`）**
- 预览里**长按网络静图**弹出底部弹窗「保存图片 / 取消」（iOS 自绘弹窗、Android BottomSheetDialog）
- 复用既有网络图保存链路，结果仍通过 `onDownloadResult` 回调；相册 Live Photo 长按仍为播放
- 顶栏在预览模式隐藏后，长按即取代原「保存到相册」按钮的入口
