...
### 提交 5–7 · `0d40bfe` / `394a700` / `9a81397` — 预览 UX 里程碑（微博式图文混排）

针对「微博帖子式图文视频混排预览」把体验补齐到对标微信的水准。**三笔提交**：契约加字段 + iOS 原生 + Android 原生。

- **契约**（`0d40bfe`）：`PickerConfig` 新增 `autoPlayVideo`（默认 `false`，**向后兼容**，公共用法不破坏；`contract_test` / 边界测试同步更新并保持全绿）。
- **飞入 hero**：点缩略图时预览从该缩略图**放大飞入**（此前是居中淡入），与已有的下拉飞回对称；翻页后关闭改为干净淡出，不再飞回过期的格子。
- **视频自动播放**：`autoPlayVideo=true` 时**仅当前页**视频自动播（相邻页/相册 Live Photo 不误播；翻走即停）。Android 顺带修了「首页就是视频时不触发自动播」的 bug。
- **加载淡入**：iOS `.opportunistic` + 首帧交叉淡入；Android 低清缩略 + 交叉淡入 —— 消除「空白闪一下」。
- **底部页码圆点**：2–8 张显示圆点、随页高亮，>8 张与视频页隐藏（避免与顶部计数重复、与播放进度条重叠）。双端一致。
- **手势**：iOS 双击可放大到点击点；Android 加了下滑 fling 关闭、缩放橡皮筋回弹、平移惯性。
- **本轮做法**：每笔都过了**多视角对抗审查**（iOS 确认并修了 2 项：飞入 hero 曾因未传 sourceFrame 成死代码、视频页圆点与进度条重叠；Android 确认并修了 1 项根因：Glide 占位色曾骗过飞入动画的「等真图」判定）。iOS 还在**模拟器**自查了呈现 / 翻页页码更新 / 加载淡入。

> 见 `DEVICE_TEST_CHECKLIST.md` 的「Q. 预览 UX 里程碑」——动画手感/配色/时长等主观项列在那里，等你真机定稿。
